    ...
    for i in range(len(boxes)):
        for j in range(i + 1, len(boxes)):
            ...
            if pa == pb and overlap(ra, rb):
                # Background rects are intentionally co-located with their labels.
                if ia.endswith("_bg") and ia[:-3] == ib:
                    continue
                if ib.endswith("_bg") and ib[:-3] == ia:
                    continue
                # Child-drop lines intentionally run up to (and are hidden by) the
                # child's label background, so ignore overlaps with the label above it.
                def _is_child_drop(vid):
                    return vid.startswith("c") and vid[1:].isdigit()
                if _is_child_drop(ia) or _is_child_drop(ib):
                    ca, cb = (ra, rb) if _is_child_drop(ia) else (rb, ra)
                    # The label is the other box; check the drop ends near the label top.
                    drop_bottom = ca[1] + ca[3]
                    if abs(ca[0] + ca[2]/2 - (cb[0] + cb[2]/2)) < 2.0 and abs(drop_bottom - cb[1]) < 4.0:
                        continue
                # Child drops also intentionally overlap the horizontal connector bar at the junction.
                if (ia.startswith("h") and _is_child_drop(ib)) or (ib.startswith("h") and _is_child_drop(ia)):
                    ha, hc = (ra, rb) if ia.startswith("h") else (rb, ra)
                    if abs(hc[1] - ha[1]) < 2.0 and ha[0] - 1 < hc[0] + hc[2]/2 < ha[0] + ha[2] + 1:
                        continue
                # Child drops also meet the vertical descender when no horizontal bar is needed.
                if (ia.startswith("v") and _is_child_drop(ib)) or (ib.startswith("v") and _is_child_drop(ia)):
                    va, vc = (ra, rb) if ia.startswith("v") else (rb, ra)
                    if abs(va[0] + va[2]/2 - (vc[0] + vc[2]/2)) < 2.0 and abs(va[1] + va[3] - vc[1]) < 2.0:
                        continue
                # Vertical descenders intentionally overlap the horizontal sibling bar at the junction.
                if (ia.startswith("h") and ib.startswith("v")) or (ia.startswith("v") and ib.startswith("h")):
                    ha, hv = (ra, rb) if ia.startswith("h") else (rb, ra)
                    hnum = ia[1:] if ia.startswith("h") else ib[1:]
                    vnum = ib[1:] if ib.startswith("v") else ia[1:]
                    if hnum == vnum and abs(hv[1] + hv[3] - ha[1] - ha[3]) < 2.0 and ha[0] - 1 < hv[0] + hv[2]/2 < ha[0] + ha[2] + 1:
                        continue
